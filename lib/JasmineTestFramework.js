        ...
        for (var i = 0; i < result.failedExpectations.length; i++) {
          ...
          if (failedExpectation.stack) {
            var reg = new RegExp(".*" + searchString + ".*"),
              match = reg.exec(failedExpectation.stack);

            if (match && match.length > 0) {
              var stackCut = indent(failedExpectation.stack.match(reg)[0].toString())

              // now append all other messages that are related to the project
              // we do that by finding the first failed "expectation" from the sanjo:jasmine package

              var stackLines = failedExpectation.stack.substring(0, match.index).split('\n');
              for (var s=stackLines.length - 1; s>=0; s--) {
                if (stackLines[s].match(/sanjo[:_]jasmine/)) {
                  break;
                }
                // match non-empty stack lines
                if (stackLines[s].match(/(^| *)at/)) {
                  stackCut = indent(stackLines[s]) + "\n" + stackCut;
                }
              }
